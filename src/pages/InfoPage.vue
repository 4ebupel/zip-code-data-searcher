...
export default {
  ...
  async mounted() {
    try {
      this.isLoading = true;
      const smth = await getInfo(this.zip).then(response => JSON.parse(response));
      this.info = smth;
...
<template>
  ...
  <section v-if="!isLoading" class="background">
    <div class="card" style="width: 18rem;" v-if="!errorStatus">
      <div class="card-body">
        <h5 class="card-title">Your City is: {{ info.location.name }}</h5>
        <h6 class="card-subtitle mb-2 text-muted">More data</h6>
        <ul class="list-group list-group-flush">
          <li class="list-group-item">Your contry is: {{ info.location.country }}</li>
          <li class="list-group-item">Your state is: {{ info.location.region }}</li>
          <li class="list-group-item">Your UTC-offset is: {{ info.location.utc_offset }}</li>
        </ul>
        <a href="/" class="btn btn-dark">Home</a>
        <a href="#" class="btn btn-dark">Toggle weather</a>
      </div>
      <div class="card-footer">
        <small class="text-muted">Observation time: {{ info.current.observation_time }}</small>
      </div>
    </div>
